    ...
    <script>
        new Vue({
            ...
            data: {
                ...
                oldTable: {!! json_encode($db->oldTable) !!},
                tableJson: ''
            },
            created() {
                console.log('oldname : ', this.oldTable);
                // If oldTable is present and valid JSON, parse it and use it to repopulate the form
                if (this.oldTable) {
                    this.table = this.oldTable;
                } else {
                    // Fallback to originalTable if oldTable is not set or fails to parse
